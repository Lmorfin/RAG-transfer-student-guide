...
**Milestone 5 — Generation and interface:** The retrieval pipeline from Milestone 4 will be connected to Groq's llama-3.3-70b-versatile model to generate grounded responses based only on the retrieved document chunks. Retrieved context and source metadata from ChromaDB will be passed into a prompt template that explicitly instructs the model to answer using only the provided information and to decline questions that are not supported by the documents. Source attribution will be generated programmatically and displayed alongside each response to improve transparency and trustworthiness. A Gradio web interface will allow users to enter transfer-related questions and view both the generated answer and the supporting source documents. The system will be tested using the evaluation-plan questions and additional out-of-scope queries to verify grounding, retrieval quality, and proper source citation.
